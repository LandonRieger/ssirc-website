...
def format_authors(entry):
    ...
    if authors[0].count(",") > 1:
        ...
        if " " in nauth[0]:
            authors = [
                ". ".join([b[0] for b in auth.split(" ")[:-1]])
                + f'. {auth.split(" ")[-1]}'
                for auth in nauth
            ]
        else:
        ...
        try:
            authors = [
                f'{a.split(",")[1].strip()} {a.split(",")[0].strip()}' for a in authors
            ]
        except:
            pass
        authors = [
            ". ".join([b[0] for b in auth.split(" ")[:-1]]) + f'. {auth.split(" ")[-1]}'
            for auth in authors
        ]
    return authors
...
def generate_publication_html(bib_file, target_year: int = None):
    ...
    if target_year:
        html_file = os.path.join(
            ...
            "templates",
            "references",
            f"references_{target_year}.html",
        )
    else:
        html_file = os.path.join(
            os.path.dirname(__file__), "templates", "references", "references.html"
        )
...
def format_ssirc_bib(bib_file):
    ...
    for lidx, line in enumerate(data):
        if (
            ("@article" in line) or ("@inbook" in line) or ("@inproceedings" in line)
        ) and lidx > 2:
            data[lidx - 2] += "}\n"

    with open(
        os.path.join(os.path.dirname(bib_file), "new_bibfile.bib"),
        "w",
        encoding="utf-8",
    ) as newfile:
...
def convert_balloon_table(filename, skiprows=0):

    data = pd.read_excel(filename, skiprows=skiprows).fillna("")
    data = data.rename(
        columns={
        ...
        for key in ["record start date", "record end date"]:
            if type(tmp[key]) is datetime.datetime:
                ...
                if tmp[key].lower() == "on going" or tmp[key].lower() == "ongoing":
                    ...
                    if "," in tmp[key]:
                        tmp[key] = datetime.datetime(
                            int(tmp[key].split(",")[0]), 1, 1
                        ).isoformat()
                    if "/" in tmp[key]:
...
def write_json(rows: list[dict], json_file: Path):
...
if __name__ == "__main__":
    ...
    base_folder = (
        Path(__file__).parent.parent / "api" / "src" / "app" / "data" / "campaigns"
    )

    rows = []
    for file in base_folder.glob("*.xlsx"):
        skiprows = 2 if file.name == "TableWG31-jpv_td_GwB_AB.xlsx" else 0
        rows += convert_balloon_table(file, skiprows=skiprows)
    write_json(
        rows,
